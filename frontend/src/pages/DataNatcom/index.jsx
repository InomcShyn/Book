...
import { API_NATCOM_DATA, API_NATCOM_DATA_2 } from "@/configs/paths/API_PATH";
...
function DataNatcom() {
  ...
  const columns = [
    ...
    {
      title: t("menu.data_source_natcom"),
  ...
  const onSearch = async () => {
    ...
    const apiUrl = value?.errorData ? API_NATCOM_DATA_2 : API_NATCOM_DATA;
  ...
  const onGetDetail = async (id) => {
    ...
    try {
      const response = await getDataApi(API_NATCOM_DATA + "/" + id, query);
      if (response.code == "00") {
  ...
  const onDelete = async () => {
    setOpenDelete(false);
    try {
      const response = await deleteDataApi(API_NATCOM_DATA, recDelete);
      if (response.code === "00") {
  ...
  const OnUpload = async (values, fileList) => {
    ...
    if (roleUser == ROLE_ACCOUNT.PARTNER) {
      formData.append("orgId", user?.userInfo?.orgId);
    } else {
  ...
  const onExport = async () => {
    ...
    try {
      const response = await exportBlobApi(API_NATCOM_DATA + "/excel", param);
  ...
  const handleSubmitForm = async (value) => {
    ...
    try {
      let response;
      if (value?.id) {
        response = await putDataApi(API_NATCOM_DATA + "/" + value.id, data);
      } else {
        response = await postDataApi(API_NATCOM_DATA, data);
      }
      if (response.code == "00") {
        toast.success(t("toast.success"));
        setOpenModal(false);
