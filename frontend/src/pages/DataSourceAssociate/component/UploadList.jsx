...
const UploadList = ({ open, onClose, recordSelected }) => {
  ...
  const onSearch = async () => {
    ...
    try {
      ...
      const response = await getDataApi(
        `${API_ORG_SOURCE}/${recordSelected.id}/files`,
        dataSearch,
      );
  ...
  const onDelete = async () => {
    ...
    try {
      const response = await deleteDataApi(
        API_ORG_SOURCE + "/files/" + recDelete,
      );
  ...
  const handleDownload = async (record) => {
    ...
    try {
      const response = await exportBlobApi(
        `${API_ORG_SOURCE}/download/${record.id}`,
      );
        ...
        const actions = [
          {
            ...
            disabledIf: () => recordSelected.reconStatus == "NOT_RECONCILED",
            onClick: () => {
