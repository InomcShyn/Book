...
import { API_MONTHLY_REPORT } from "@/configs/paths/API_PATH";
...
function ReportMonthly() {
  ...
  const columns = [
    ...
    {
      title: t("report_daily.trans_month"),
      dataIndex: "transDate",
      key: "transDate",
      render: (transDate) => convertDate(transDate, "YYYY-MM"),
    },
    {
  ...
  const onSearch = async () => {
    ...
    try {
      setLoading(true);
      const response = await getDataApi(API_MONTHLY_REPORT, dataSearch);
      if (response.code == "00") {
  ...
  const onExport = async () => {
    ...
    try {
      ...
      exportExcel(response, "Report_monthly_" + time);
