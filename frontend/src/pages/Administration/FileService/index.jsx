...
import { API_ORG_TEMPLATES } from "@/configs/paths/API_PATH";
...
import ActionCreateEditService from "./component/ActionCreateEditService";
...
function FileService() {
  ...
  const typeService = typeFileService(t);
  ...
  const columns = [
    ...
    {
      title: t("file_service.service_name"),
      dataIndex: "serviceName",
      key: "serviceName",
    },
    {
      title: t("file_service.type"),
      dataIndex: "type",
      key: "type",
      render: (status) => {
        const lable = typeService.find((item) => item.value === status)?.label;
        return <p>{lable}</p>;
      },
    },
    {
      title: t("file_service.ORG_name"),
      dataIndex: "orgName",
      key: "orgName",
    },
    {
      title: t("file_service.recon_template_name"),
      dataIndex: "reconTemplateName",
      key: "reconTemplateName",
    },
    {
      title: t("file_service.exchange_rate"),
    ...
    {
      title: t("file_service.create_time"),
    ...
    {
      title: t("file_service.update_time"),
  ...
  const onSearch = async (value) => {
    ...
    try {
      setLoading(true);
      const response = await getDataApi(API_ORG_TEMPLATES, dataSearch);
      if (response.code == "00") {
  ...
  const onGetDetail = async (id) => {
    try {
      const response = await getDataApi(API_ORG_TEMPLATES + "/" + id);
      if (response.code == "00") {
  ...
  const onDelete = async () => {
    setOpenDelete(false);
    try {
      const response = await deleteDataApi(API_ORG_TEMPLATES + "/" + recDelete);
      if (response.code == "00") {
  ...
  const handleSubmitForm = async (value) => {
    ...
    try {
      let response;
      if (value?.id) {
        response = await putDataApi(API_ORG_TEMPLATES + "/" + value.id, data);
      } else {
        response = await postDataApi(API_ORG_TEMPLATES, data);
      }
      if (response.code == "00") {
