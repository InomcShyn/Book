...
function Service() {
  ...
  const columns = [
    ...
    {
      title: t("service.service_name"),
      dataIndex: "serviceName",
      key: "serviceName",
    },
    {
      title: t("service.service_code"),
      dataIndex: "serviceCode",
      key: "serviceCode",
    },
    {
      title: t("service.default_currency"),
    ...
    },
    {
      ...
      key: "type",
      render: (type) => {
        const lable = types.find((item) => item.value === type)?.label;
        return <p>{lable}</p>;
      },
    },
    {
      title: t("service.max_deviation_percentage"),
      dataIndex: "maxDeviationPercentage",
  ...
  const onSearch = async (value) => {
    ...
    try {
      const response = await getDataApi(API_SERVICE, dataSearch);
      if (response.code == "00") {
  ...
  const onDelete = async () => {
    setOpenDelete(false);
    try {
      const response = await deleteDataApi(API_SERVICE + "/" + recDelete);
      if (response.code == "00") {
        toast.success(t(MessageSuccess));
        onSearch();
      } else toast.error(t(MessageError) || response?.message);
    } catch (error) {
  ...
  const handleSubmitForm = async (value) => {
    ...
    try {
      let response;
      if (value?.id) {
        response = await putDataApi(API_SERVICE + "/" + value.id, data);
      } else {
        response = await postDataApi(API_SERVICE, data);
      }
      if (response.code == "00") {
